...
| Loop In LEDs | `0x90/0x10`, `0x91/0x10` | active audio loop exists | P4 `audio_engine` loop state | Implemented output for active loop state only | Behavior smoke passed D1/D2 2026-06-21 for active loop state; output probe verified 2026-06-20; pending decision: 2026-06-26 smoke showed `IN` alone does not light the LED before `OUT` creates an active loop |
