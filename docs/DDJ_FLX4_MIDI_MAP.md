...
| Beat FX on/off | CH1/global `0x94/0x47`, CH2 `0x95/0x47` | press/release; press toggles P4 Beat FX enabled state | FX channel selector | `CTRL_ID_BEAT_FX_ON` / `LED_BEAT_FX_ON` | P4 Beat FX state model + LED feedback | Implemented state/mapping; FILTER DSP implemented; ON/OFF LED follows P4 state; Echo/delay deferred | Host-tested from XML; LED feedback host-tested; hardware smoke passed 2026-07-01 |
| Beat FX on/off + Shift | CH1/global `0x94/0x43`, CH2 `0x95/0x43` | press/release | shifted FX channel selector | `CTRL_ID_BEAT_FX_CLEAR` | P4 Beat FX state reset | Implemented state/mapping; Echo/delay DSP deferred | Host-tested from XML; hardware smoke passed 2026-07-01 |
