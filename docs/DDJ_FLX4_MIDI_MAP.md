...
| Beat FX select next / previous | `0x94/0x63`, `0x94/0x64` | press/release | FX section | `CTRL_ID_BEAT_FX_SELECT_NEXT`, `CTRL_ID_BEAT_FX_SELECT_PREV` | P4 Beat FX state model | Implemented state/mapping | Host-tested from XML; hardware smoke pending |
...
| Beat FX on/off | CH1/global `0x94/0x47`, CH2 `0x95/0x47` | press/release; press toggles P4 Beat FX enabled state | FX channel selector | `CTRL_ID_BEAT_FX_ON` | P4 Beat FX state model | Implemented state/mapping; audio DSP deferred | Host-tested from XML; hardware smoke pending |
