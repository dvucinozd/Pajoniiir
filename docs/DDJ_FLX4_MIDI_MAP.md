...
| Beat Jump shifted size pads | D1 `0x98/0x26..0x27`, D2 `0x9A/0x26..0x27` | press/release | shifted Beat Jump mode | pad action with size inc/dec | global P4 Beat Jump page: fractional / default / large | Implemented on `master` | Host-tested from XML; physical acceptance pending |
...
| Shift + Loop In adjust LEDs | official list/PDF output: `0x90/0x4C`, `0x91/0x4C` | selected loop-in jog-edit mode | P4 loop-adjust state, restored on reconnect | Stateful on `master` | Host-tested; physical jog-edit acceptance pending |
...
| Shift + Loop Out adjust LEDs | official list/PDF output: `0x90/0x4E`, `0x91/0x4E` | selected loop-out jog-edit mode | P4 loop-adjust state, restored on reconnect | Stateful on `master` | Host-tested; physical jog-edit acceptance pending |
| Hot Cue pad LEDs | normal D1 `0x97/0x00..0x07`, normal D2 `0x99/0x00..0x07`; shifted mirror D1 `0x98/0x00..0x07`, D2 `0x9A/0x00..0x07` | hot cue slot exists while Hot Cue mode is selected | P4 hot cue store state | Normal output implemented; FLX4-gated shifted mirror implemented on `master` | Normal hardware smoke passed 2026-07-01; shifted mirror host-tested, physical acceptance pending |
| Pad FX1 pad LEDs | normal D1 `0x97/0x10..0x17`, normal D2 `0x99/0x10..0x17`; shifted mirror D1 `0x98/0x10..0x17`, D2 `0x9A/0x10..0x17` | momentary active Pad FX1 pad while Pad FX1 mode is selected | P4 `deck_core` Pad FX momentary state | Normal output implemented; FLX4-gated shifted mirror implemented on `master` | Normal hardware smoke passed 2026-07-01; shifted mirror host-tested, physical acceptance pending |
...
| Pad FX2 pad LEDs | normal D1 `0x97/0x50..0x57`, normal D2 `0x99/0x50..0x57`; shifted mirror D1 `0x98/0x50..0x57`, D2 `0x9A/0x50..0x57` | momentary active Pad FX2 pad while Pad FX2 mode is selected | P4 `deck_core` Pad FX momentary state | Normal output implemented; FLX4-gated shifted mirror implemented on `master` | Normal hardware smoke passed 2026-07-01; shifted mirror host-tested, physical acceptance pending |
| Beat Loop pad LEDs | normal D1 `0x97/0x60..0x67`, normal D2 `0x99/0x60..0x67`; shifted mirror D1 `0x98/0x60..0x67`, D2 `0x9A/0x60..0x67` | P4-owned active Beat Loop pad while Beat Loop mode is selected | P4 loop state, selected Beat Loop pad, and pad mode | Normal output implemented; FLX4-gated shifted mirror implemented on `master` | Normal hardware smoke passed 2026-07-01; shifted mirror host-tested, physical acceptance pending |
| Beat Jump pad LEDs | normal D1 `0x97/0x20..0x27`, normal D2 `0x99/0x20..0x27`; shifted mirror D1 `0x98/0x20..0x27`, D2 `0x9A/0x20..0x27` | P4 `audio_engine_deck_status_t.loaded` while Beat Jump mode is selected | P4 `deck_core` publishes normal state; direct host mirrors it only for FLX4 VID:PID | Normal output implemented; full shifted mirror implemented on `master` | Normal hardware smoke passed 2026-07-07; shifted mirror host-tested, physical acceptance pending |
