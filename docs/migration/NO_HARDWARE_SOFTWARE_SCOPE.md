...
- local P4 profile activation and reconnect replay tests;
- local P4 LED runtime preserving dynamic-profile priority, built-in fallback,
  firmware-authoritative Track Load IDs, VU packets and blink values;
- ESP32-P4-only integration harness combining host manager, MSC, MIDI, local
  semantic translation, profile selection, LED mapping and a separate dispatch
  task;
- portable USB-MIDI codec, event-buffer, controller-runtime, profile and LED
  tests;
...
physical LED transport and FLX4 USB Audio cue remain the known-good product path.
...
- physical MIDI LED output and reconnect snapshot on DDJ-FLX4;
...
Selective per-port power/recovery is implemented and software-validated in the
