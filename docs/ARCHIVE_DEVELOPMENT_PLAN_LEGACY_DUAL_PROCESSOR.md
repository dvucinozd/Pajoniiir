...
> **ARHIVIRANI DUAL-PROCESSOR LEDGER.** Ovo je puni povijesni fazni zapis.
> Aktivni P4-only plan nalazi se na izvornom putu `docs/DEVELOPMENT_PLAN.md`
> nakon arhivskog preimenovanja ovog dokumenta.
...
- map MVP controls to the IDs in `docs/ARCHIVE_CONTROL_LINK_PROTOCOL.md`; done
...
- **0xA6 bulk transport.** A variable-length frame layer on the same UART
  ...
  CRC32-verified reassembly (`cp_xfer.c`). See `docs/ARCHIVE_CONTROL_LINK_PROTOCOL.md`.
...
hardening. See `docs/ARCHIVE_OTA_UPDATE_PLAN_DUAL_TARGET.md` for the acceptance record.
...
recorded in `ARCHIVE_POST_R5_PLAN_DUAL_TARGET.md`; E1 signed OTA is complete and E2 enclosure wiring
and service readiness is next.

- The call graph, compatibility-path corrections and ESP-IDF size baseline are
  recorded in `ARCHIVE_R5_DEAD_CODE_AUDIT.md`.
...
- ~~96 kHz/24-bit FLAC fails to load on deck 1~~ — **withdrawn: not a defect.**
  ...
  with no file behind it. FLAC decoding works. See `ARCHIVE_BENCH_NOTES_LEGACY_DUAL_PROCESSOR.md` for why
