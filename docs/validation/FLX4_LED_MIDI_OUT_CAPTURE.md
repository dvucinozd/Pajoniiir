...
| Beat FX On/Off | global/deck | audio_fx rack | `0x94` / `0x95` | note `0x47` | `0x00` | `0x7F` | implemented | passed 2026-07-01 | physically verified 2026-06-20; P4 state smoke verified 2026-07-01 | Beat FX ON/OFF LED follows P4 Beat FX enabled state; clear/reset forces off. |
...
| 21 | Press and release Pad FX1/Pad FX2 pads while the matching pad mode is selected | Normal Pad FX pad LED lights while the pad is held and turns off on release; Pad FX Echo audio tail continues after release | pass 2026-07-01 | Operator confirmed Pad FX pad LEDs worked on hardware. Firmware drives the normal `0x97`/`0x99` Pad FX pad LED ranges from P4-owned momentary pad state; shifted mirror LED ranges remain deferred. |
| 22 | Set and clear a Hot Cue pad while Hot Cue mode is selected | Normal Hot Cue pad LED lights when the slot exists and turns off after Shift+pad clear | pass 2026-07-01 | Operator confirmed Hot Cue pad LED feedback worked on hardware after flashing commit `af56cd5`. Firmware drives the normal `0x97`/`0x99` Hot Cue pad LED ranges from P4-owned hot cue store state; shifted mirror LED ranges remain deferred. |
| 23 | After flashing commit `c1b9aa1b`, select supported pad modes | Hot Cue, Pad FX1/2, Beat Jump, and Beat Loop mode LEDs continue to follow selected supported mode | pass 2026-07-07 | Operator confirmed supported pad-mode LEDs still work after removing Keyboard/Stems, Sampler, and Key Shift from product scope. |
| 24 | After flashing commit `c1b9aa1b`, press removed pad modes | Sampler, Keyboard/Stems, and Key Shift do not change active mode and do not latch their mode LEDs on | pass 2026-07-07 | Operator confirmed removed pad modes are inert. Numeric IDs remain reserved for compatibility/off output. |
| 25 | Exercise Play+Shift / Censor LED output | Censor LED follows the P4-owned censor state | pass 2026-07-07 | Operator confirmed LED batch smoke passed. |
| 26 | Exercise Cue+Shift / track-start LED output | Cue+Shift / track-start output packet drives the expected FLX4 LED | pass 2026-07-07 | Output mapping is hardware-smoked; persistent snapshot state remains deferred. |
| 27 | Exercise Loop Adjust In/Out LED outputs | Loop Adjust In and Loop Adjust Out output packets drive the expected FLX4 LEDs | pass 2026-07-07 | Output mapping is hardware-smoked; persistent snapshot state remains deferred. |
| 28 | Exercise Track Load Deck 1/2 illumination outputs | Track Load Deck 1 and Deck 2 output packets drive the expected FLX4 illumination | pass 2026-07-07 | Output mapping is hardware-smoked; deck-loaded snapshot ownership remains deferred. |
| 23 | Trigger normal Beat Loop pads on Deck 1 and Deck 2, then exit with `RELOOP/EXIT` | Normal Beat Loop pad LED follows the pad that set the active loop; `RELOOP/EXIT` clears the loop and pad LED | pass 2026-07-01 | Initial smoke found that LED inference by loop duration failed on non-120-BPM material. Firmware now tracks P4-owned active Beat Loop pad state; operator confirmed Beat Loop pad LEDs and physical `RELOOP/EXIT` worked on both decks. |
| 24 | Hold and release shifted Beat Loop pads on both decks | Shifted Beat Loop acts as momentary loop-roll: press sets temporary loop and release restores previous loop or clears when none existed | pass 2026-07-01 | Operator reported shifted Beat Loop behavior had already been tested and worked. Shifted mirror pad LED output remains deferred. |
| 25 | Trigger Beat Jump pads 1-8 on Deck 1 and Deck 2 | Pads jump by the mapped beat offsets and playback/controller responsiveness remain normal | pass 2026-07-01 | Operator confirmed Beat Jump pads worked on both decks. Current implemented offsets are pads 1-4 backward `-32/-16/-8/-4` beats and pads 5-8 forward `+4/+8/+16/+32` beats. |
