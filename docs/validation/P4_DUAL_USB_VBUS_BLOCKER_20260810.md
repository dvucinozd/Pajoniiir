...
1. Use one USB 2.0 data interposer per port. Keep D+, D-, ground and shield
   continuous; isolate the P4-side VBUS conductor from downstream VBUS.
...
4. For a TPS2561 implementation, use one output per port, local input/output
   ...
   current limit. A 69.8 kOhm `ILIM` resistor targets approximately 0.8 A per
   channel; verify the exact populated part and its datasheet before assembly.
...
together; the provisional bench target is a regulated 5 V / 3 A source with
per-port over-current protection. This is a design target, not a measured
acceptance result.

## Project impact

Development on `feat/p4-dual-usb-host` is intentionally paused at commit
`069ef9d` plus this documentation update. Do not merge the branch, remove the
S3 fallback or claim dual-USB hardware acceptance until the protected VBUS
arrangement is built, electrically checked and the full dual-device matrix is
run. Normal work may continue from `master` without this experimental branch.
