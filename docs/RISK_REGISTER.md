...
| P1 | USB0 media does not recover after boot, remove or active decode fault | Groups A--H pass: cold/warm boots, both insertion orders, five idle removals, five deterministic Library-load removals, five deterministic audio-load removals and five idle FLX4 reconnects; progress is 36/50 cycles | Complete active USB1 and held-control lifecycle groups plus reboot/OTA recovery |
| P1 | USB1 FLX4 control/audio recovery loses USB0, repeats recovery or leaves controls latched | Group H passed 5/5 on exact image `RC2-128-g495947e`: USB0 and the 100-track Library remained stable and FLX4 profile, MIDI, LEDs, UAC and audible MAIN/cue recovered each time | Run USB1 reconnect during dual playback, then held-control saturation, LED resync and UAC recovery |
