...
/* Measured on hardware: the first power-on does not re-enumerate a drive that
 * was already attached across the reset, and lengthening the off window does not
 * help (120 ms mounted at ms=7005, 400 ms at ms=7846 — both via the retry, not
 * the first cycle). The likely reason is that the root port's logical power
 * state does not actually cut VBUS on this board, so the drive never loses
 * power and never resets itself. What does work is repeating the port cycle, so
 * keep the off window short and let the retry converge quickly instead. */
#define ROOT_PORT_SETTLE_MS   150
#define ROOT_PORT_RETRY_MS    900   /* wait for a connect before cycling again */
#define ROOT_PORT_MAX_CYCLES  8     /* then fall back to the slow retry below */
