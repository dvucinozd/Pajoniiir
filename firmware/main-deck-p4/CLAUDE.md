...
| `library` (USB) | ✅ **RUNNING ON HW** | `library_init()` transactionally publishes immutable track records from `/usb`; sort republishes only a compact double-buffered `uint16_t` row order; `library_get_summary()` is the copy accessor (`library_get_ptr()` is simulator-only) |
