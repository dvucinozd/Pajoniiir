...
- **True scratch / "vinyl mode"** — while playing, the jog currently *nudges* the tempo (pitch bend); it does not scratch the audio like vinyl (touch the platter → drag playback back/forth with real reverse). Needs a scratch DSP that reads a PSRAM scratch buffer bidirectionally at the jog velocity while the platter is touched (`JOG_TOUCH` gates scratch vs bend), plus reverse playback support. **Detailed phased plan: [`docs/VINYL_SCRATCH_PLAN.md`](../../docs/VINYL_SCRATCH_PLAN.md).**
