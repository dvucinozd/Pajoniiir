...
int p4_ctrl_id_s3_boot_challenge(void);
int p4_ctrl_id_s3_boot_ack(void);
...
int p4_ctrl_id_deck1_loop_size(void);
int p4_ctrl_id_deck2_loop_size(void);
...
int p4_ctrl_deck_ext_action_sync_off(void);
int p4_led_cue(void);
...
int s3_ctrl_id_s3_boot_challenge(void);
int s3_ctrl_id_s3_boot_ack(void);
...
int s3_ctrl_id_deck1_loop_size(void);
int s3_ctrl_id_deck2_loop_size(void);
...
int s3_ctrl_deck_ext_action_sync_off(void);
int s3_led_cue(void);
...
static void test_s3_and_p4_deck_aware_ids_match(void)
{
    ...
    assert(s3_ctrl_id_deck1_loop_size() == p4_ctrl_id_deck1_loop_size());
    assert(s3_ctrl_id_deck1_loop_size() == CTRL_ID_DECK1_LOOP_SIZE);
    assert(s3_ctrl_id_deck2_loop_size() == p4_ctrl_id_deck2_loop_size());
    assert(s3_ctrl_id_deck2_loop_size() == CTRL_ID_DECK2_LOOP_SIZE);
    assert(CTRL_ID_DECK1_EXT_ACTION == 0x2C);
    assert(CTRL_ID_DECK1_LOOP_SIZE == 0x2D);
    ...
    assert(s3_ctrl_deck_ext_action_sync_off() == p4_ctrl_deck_ext_action_sync_off());
    ...
    assert(CTRL_DECK_EXT_ACTION(CTRL_DECK_EXT_VALUE(CTRL_DECK_EXT_ACTION_SYNC_OFF, true)) ==
...
static void test_s3_and_p4_flx4_connection_state_ids_match(void)
{
    ...
    assert(s3_ctrl_id_s3_boot_challenge() == p4_ctrl_id_s3_boot_challenge());
    assert(s3_ctrl_id_s3_boot_challenge() == CTRL_ID_S3_BOOT_CHALLENGE);
    assert(s3_ctrl_id_s3_boot_challenge() == 0x86);
    assert(s3_ctrl_id_s3_boot_ack() == p4_ctrl_id_s3_boot_ack());
    assert(s3_ctrl_id_s3_boot_ack() == CTRL_ID_S3_BOOT_ACK);
    assert(s3_ctrl_id_s3_boot_ack() == 0x87);
