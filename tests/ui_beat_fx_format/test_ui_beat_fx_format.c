...
static void test_delay_effect_formats_label(void)
{
    deck_core_beat_fx_state_t state = {
        .effect = DECK_CORE_BEAT_FX_DELAY,
        .beat = DECK_CORE_BEAT_FX_BEAT_2,
        .target = CTRL_BEAT_FX_TARGET_BOTH,
        .depth = 96,
        .enabled = true,
    };
    ...
    assert(strcmp(text.effect, "DELAY") == 0);
    assert(strcmp(text.beat, "2") == 0);
    assert(strcmp(text.target, "BOTH") == 0);
    assert(strcmp(text.enabled, "FX ON") == 0);
}
...
int main(void)
{
    ...
    test_delay_effect_formats_label();
